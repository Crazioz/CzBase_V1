			...
			if Config.ThirdEyeName == 'ox_target' then
				...
				exports.ox_target:addBoxZone({
				    ...
				    options = {
					{
					    ...
					    label = "Utiliser l'ascenseur depuis " .. floor.level,
					    onSelect = function()
        ...
        for elevatorName, elevatorFloors in pairs(Config.Elevators) do
            for index, floor in pairs(elevatorFloors) do
                ...
                if distance <= 10.0 then
                    ...
                    if distance <= 5 then
			sleep = 0
                        DrawText3Ds(floor.coords.x,floor.coords.y,floor.coords.z, "Appuyer sur ~r~E~w~ pour utiliser l'ascenseur depuis" .. floor.level)
...
function NotifyNoAccess()
	AddTextEntry('elevatorHelp', 'Vous ne pouvez pas utiliser ceci !')
	BeginTextCommandDisplayHelp('elevatorHelp')
	EndTextCommandDisplayHelp(0, false, true, -1)
end
