...
AddEventHandler("utk_fh:outcome", function(oc, arg)
    ...
    elseif not oc then
        ESX.ShowNotification("Il vous manque quelque chose !", "info", 3000)
    end
end)
            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Piller l'argent", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Piller l'argent", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Piller l'argent", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        ESX.ShowNotification("L'alarme s'active", "info", 3000)
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Déverrouiller la porte", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Verrouiller la porte", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    ESX.ShowNotification("Fermeture du coffre fort dans 30 secondes !", "info", 3000)

    Citizen.Wait(30000)
    ESX.ShowNotification("Fermeture du coffre fort !", "info", 3000)
    ESX.ShowNotification("L'alarme s'active", "info", 3000)
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['progressBars']:startUI(2000, "Utilisation d'une carte malveillante")
    Citizen.Wait(1500)
    ...
    ESX.ShowNotification("Hack reussie", "info", 3000)
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    ESX.ShowNotification("Tu a 2 minutes avant que le système de sécurité s'active !", "info", 3000)
...
function SecondDoor(data, key)
    ...
    ESX.ShowNotification("Accès reussie", "info", 3000)
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
                ...
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            ESX.ShowNotification("Vous n'avez pas de carte de sécurité", "info", 3000)
                        end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] Commencer le braquage de banque", 0.40)
