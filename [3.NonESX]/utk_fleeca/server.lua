        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Cette banque a récemment volé. Vous devez attendre"..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Cette banque est actuellement braquer.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "Vous n'avez pas de carte d'accès malveillante.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Il n'y a pas assez de policiers dans la ville.")
    end
end)
