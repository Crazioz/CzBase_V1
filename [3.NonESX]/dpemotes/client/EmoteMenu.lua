...
function AddEmoteMenu(menu)
    ...
    local tactiquemenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['tactiqueemotes'], "", "", Menuthing, Menuthing)
    ...
    if Config.TactiqueEmotesEnabled then 
    for a, b in pairsByKeys(DP.Tactique) do
        x, y, z = table.unpack(b)
        tactiqueitem = NativeUI.CreateItem(z, "/e (" .. a .. ")")
        tactiquemenu:AddItem(tactiqueitem)
        table.insert(TactiqueTable, a)
        if not Config.SqlKeybinding then
            favEmotes[a] = z
        end
    end
    ...
    if Config.TactiqueEmotesEnabled then
        tactiquemenu.OnItemSelect = function(sender, item, index)
            EmoteMenuStart(TactiqueTable[index], "tactique")
        end
    end
        ...
        if input ~= nil then
            ...
            if #results > 0 then
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    elseif data.table == "TactiqueEmotes" then
                        EmoteMenuStart(data.name, "tactique")
