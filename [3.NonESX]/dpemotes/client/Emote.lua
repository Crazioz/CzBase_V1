...
Citizen.CreateThread(function()
    ...
    if Config.SqlKeybinding then
        TriggerEvent('chat:addSuggestion', '/emotebind', 'Bind une animation sur une touche',
            { { name = "key", help = "num4, num5, num6, num7. num8, num9. Numpad 4-9!" },
                { name = "emotename", help = "dance, camera, sit or any valid emote." } })
        TriggerEvent('chat:addSuggestion', '/emotebinds', 'Voir les toucher déjà bind.')
    end
    TriggerEvent('chat:addSuggestion', '/emotemenu', 'Ouvre le menu des emotes.')
    TriggerEvent('chat:addSuggestion', '/walk', 'permet de changer de démarche.',
        { { name = "style", help = "/walks for a list of valid styles" } })
    TriggerEvent('chat:addSuggestion', '/emotecancel', 'Annule l\'animation en cours.')
end)
...
function EmoteMenuStart(args, hard)
    ...
    elseif etype == "tactique" then
        if DP.Tactique[name] ~= nil then
            if OnEmotePlay(DP.Tactique[name]) then end
        end
...
function EmoteCommandStart(source, args, raw)
    if #args > 0 then
        ...
        elseif DP.Dances[name] ~= nil then
            if OnEmotePlay(DP.Dances[name]) then end
            return
        elseif DP.Tactique[name] ~= nil then
            if OnEmotePlay(DP.Tactique[name]) then end
            return
