...
Citizen.CreateThread(function()
    ...
    for k, v in pairs (VehicleShop) do
        ...
        AddTextComponentSubstringPlayerName("Magasin de véhicules : "..v.name.."")
...
function Marker(vec,msg,event,server,dist)
    ...
    while #(vec - GetEntityCoords(PlayerPedId())) < dist and neargarage do
        ...
        ShowFloatingHelpNotification("Appuyez [E] "..msg,vec,r)
        ...
        for k,v in pairs(Refund) do
            ...
            while dist < v.Dist * 2 and inveh do
                ...
                if dist < v.Dist and inveh then
                    neargarage = true
                    if Config.Marker then
                        Marker(vec,"Vendre un véhicule",'renzu_vehicleshop:sellvehicle',true,3)
                    elseif Config.UsePopUI then
                ...
                if Config.Licensed and not Config.framework == 'QBCORE' then
                    TriggerServerCallback_('esx_license:checkLicense', function(cb)
                        if cb then
                            ...
                            ShowNotification("Vous n'avez pas de permis de conduire")
                        end
...
RegisterNUICallback("choosebrands", function(data, cb)
    ...
    if cars > 0 then
        ...
        ShowNotification("Aucun véhicule n'est disponible")
    end
end)
...
function OpenShop(id)
    ...
    if cars > 0 then
        ...
        ShowNotification("Aucun véhicule n'est disponible 2")
    end
end
    ...
    for k,v in pairs(VehicleShop) do
        ...
        if dist <= 40.0 and id == v.name then
            ...
            if not HasModelLoaded(hash) then
                ...
                BusyspinnerOff()
                Wait(10)
                AddTextEntry("CUSTOMLOADSTR", 'Téléchargement des actifs du véhicule..')
                ...
                while not HasModelLoaded(hash) do
                    ...
                    if c > 20000 then
                        BusyspinnerOff()
                        Wait(10)
                        AddTextEntry("CUSTOMLOADSTR", 'Le téléchargement du véhicule prend trop de temps, demandez à l\'administrateur du serveur..')
                        BeginTextCommandBusyspinnerOn("CUSTOMLOADSTR")
                        EndTextCommandBusyspinnerOn(4)
                        Wait(2000)
                        break
    ...
    TriggerServerCallback_("renzu_vehicleshop:GenPlate",function(plate)
        ...
        TriggerServerCallback_("renzu_vehicleshop:buyvehicle",function(canbuy)
            if canbuy then
                ...
                CloseNui()
                ShowNotification("Achat réussi : Plaque: "..props.plate.."")
    ...
    function(data, cb)
        ...
        if Config.EnableTestDrive and type ~= 'plane' then
            ...
            TriggerServerCallback_("renzu_vehicleshop:GenPlate",function(plate)
                ...
                DoScreenFadeIn(3000)
                ShowNotification("Essai routier : Démarrer")
                SetEntityAlpha(v, 255, false)
            ...
            SendNUIMessage(
            {
                type = "notify",
                typenotify = 'error',
                message = "Le test de conduite est désactivé",
                }
            ) 
        end
    end
