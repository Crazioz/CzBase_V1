...
function Deleteveh(plate,src)
    ...
    if plate and type(plate) == 'string' then
        ...
        print('erreur  not string - Supprimer le véhicule')
    end
end
...
AddEventHandler('renzu_vehicleshop:sellvehicle', function()
    ...
    if r and #r > 0 then
        ...
        if model == GetEntityModel(vehicle) then
            ...
            print("EXPLOIT")
            xPlayer.showNotification('Êtes-vous vraiment sûr que c\'est le véhicule?',1,0,110)
        end
    else
        xPlayer.showNotification('Vous n\'êtes pas propriétaire de ce véhicule',1,0,110)
    end
end)
...
function Buy(result,xPlayer,model, props, payment, job, type, garage, notregister)
    ...
    if result then
        ...
        if money then
            ...
            xPlayer.showNotification('Pas assez d\'argent',1,0,110)
            fetchdone = true
            bool = false
        end
    else
        print("VEHICLE NOT IN DATABASE or CONFIG")
        xPlayer.showNotification('Le véhicule n\'existe pas',1,0,110)
        fetchdone = true
        bool = false
    end
