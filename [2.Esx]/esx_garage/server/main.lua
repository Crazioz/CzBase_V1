...
AddEventHandler('esx_garage:updateOwnedVehicle', function(stored, parking, Impound, data, spawn)
	...
	local xPlayer  = ESX.GetPlayerFromId(source)
		MySQL.update('UPDATE owned_vehicles SET `stored` = @stored, `parking` = @parking, `pound` = @Impound, `vehicle` = @vehicle WHERE `plate` = @plate AND `owner` = @identifier OR `plate` = @plate AND `owner` = @job',
		{
...
AddEventHandler('esx_garage:setImpound', function(Impound, vehicleProps)
	...
	local xPlayer  = ESX.GetPlayerFromId(source)
		...
		MySQL.update('UPDATE owned_vehicles SET `stored` = @stored, `pound` = @Impound, `vehicle` = @vehicle WHERE `plate` = @plate AND `owner` = @identifier OR `plate` = @plate AND `owner` = @job',
		{
			['@identifier'] = xPlayer.identifier,
			['@job']		= xPlayer.job.name,
...
ESX.RegisterServerCallback('esx_garage:getVehiclesInParking', function(source, cb, parking)
	...
	MySQL.query('SELECT * FROM `owned_vehicles` WHERE `owner` = @identifier AND `parking` = @parking AND `stored` = 1 OR `owner` = @job AND `parking` = @parking AND `stored` = 1',
	{
		['@identifier'] 	= xPlayer.identifier,
		['@job']			= xPlayer.job.name,
		['@parking']     	= parking
...
ESX.RegisterServerCallback('esx_garage:checkVehicleOwner', function(source, cb, plate)
	...
	MySQL.query('SELECT COUNT(*) as count FROM `owned_vehicles` WHERE `owner` = @identifier AND `plate` = @plate OR `owner` = @job AND `plate` = @plate',
	{
		['@identifier'] 	= xPlayer.identifier,
		['@job']			= xPlayer.job.name,
		['@plate']     		= plate
...
ESX.RegisterServerCallback('esx_garage:getVehiclesImpounded', function(source, cb)
	...
	MySQL.query('SELECT * FROM `owned_vehicles` WHERE `owner` = @identifier AND `stored` = 0 OR `owner` = @job AND `stored` = 0',
	{
		['@identifier'] 	= xPlayer.identifier,
		['@job']			= xPlayer.job.name,
	}, function(result)
...
ESX.RegisterServerCallback('esx_garage:getVehiclesInPound', function(source, cb, Impound)
	...
	MySQL.query('SELECT * FROM `owned_vehicles` WHERE `owner` = @identifier AND `pound` = @Impound AND `stored` = 2 OR `owner` = @job AND `pound` = @Impound AND `stored` = 2',
	{
		['@identifier'] 	= xPlayer.identifier,
		['@job']			= xPlayer.job.name,
		['@Impound']     	    = Impound
